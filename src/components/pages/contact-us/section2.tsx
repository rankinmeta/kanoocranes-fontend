  ...
  return (
    <section className="container container-padding-x py-10 md:py-20 grid md:grid-cols-5 gap-8 md:gap-5">
      ...
      <div className="grid lg:grid-cols-2 gap-3 md:col-span-3">
        {phone && <Card label="Phone Number" value={phone} icon={<Phone />} />}
        {email && <Card label="Email Address" value={email} icon={<Mail />} />}
        {whatsapp && (
          <Card label="WhatsApp" value={whatsapp} icon={<Whatsapp />} />
        )}
        {address && <Card label="Address" value={address} icon={<PinDrop />} />}
      </div>
    </section>
  );
};
