					...
					<>
						<div className="md:col-span-3 flex md:grid grid-cols-3 gap-3 overflow-x-scroll scrollbar-none h-fit">
							{currentModels?.map((model: ModelProps) => (
								<Card
									crane_capacity={
										model.main_section.crane_capacity
									}
									image={model.main_section.images[0]}
									listingType={model.main_section.listingType}
									max_lifting_height={
										model.main_section.max_lifting_height
									}
									max_working_radius={
										model.main_section.max_working_radius
									}
						...
						{currentModels?.length > 6 && <Pagination className="mt-10 md:col-span-4 flex justify-center w-full">
							<PaginationContent>
								<PaginationItem>
									<PaginationPrevious
										href="#"
